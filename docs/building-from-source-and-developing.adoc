 ...
 * https://cmake.org/[CMake] (setup on users path environmental variable)
...
.*Recommended for C++ development (All Platforms)*
 * https://code.visualstudio.com/[Visual Studio Code] with the following Extensions (Recommended)
    ...
    ** https://marketplace.visualstudio.com/items?itemName=webfreak.debug[Debugging of NI-Linux-RT Targets with `gdbserver`]
...
WARNING: All the tooling and scripts assume the that `vcpkg` is installed in `C:\src\vcpkg`
